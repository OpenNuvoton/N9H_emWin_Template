...
static uint8_t lowByte(uint16_t ww)
{
  return (uint8_t) ((ww) & 0x00FF);
}


static uint8_t highByte(uint16_t ww)
{
  return (uint8_t) ((ww) >> 8);
}

static uint16_t word(uint8_t lobyte, uint8_t hibyte)
{
  return (uint16_t) (hibyte <<8 )| lobyte;
}
