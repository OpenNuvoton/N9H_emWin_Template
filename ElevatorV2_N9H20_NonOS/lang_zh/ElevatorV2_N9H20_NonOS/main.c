...
extern GUI_CONST_STORAGE GUI_BITMAP bmu26;
extern GUI_CONST_STORAGE GUI_BITMAP bmu27;
extern GUI_CONST_STORAGE GUI_BITMAP bmu28;
extern GUI_CONST_STORAGE GUI_BITMAP bmu29;

extern GUI_CONST_STORAGE GUI_BITMAP bmu30;
extern GUI_CONST_STORAGE GUI_BITMAP bmu31;
extern GUI_CONST_STORAGE GUI_BITMAP bmu32;
extern GUI_CONST_STORAGE GUI_BITMAP bmu33;
extern GUI_CONST_STORAGE GUI_BITMAP bmu34;
...
extern GUI_CONST_STORAGE GUI_BITMAP bmd26;
extern GUI_CONST_STORAGE GUI_BITMAP bmd27;
extern GUI_CONST_STORAGE GUI_BITMAP bmd28;
extern GUI_CONST_STORAGE GUI_BITMAP bmd29;

extern GUI_CONST_STORAGE GUI_BITMAP bmd30;
extern GUI_CONST_STORAGE GUI_BITMAP bmd31;
extern GUI_CONST_STORAGE GUI_BITMAP bmd32;
extern GUI_CONST_STORAGE GUI_BITMAP bmd33;
extern GUI_CONST_STORAGE GUI_BITMAP bmd34;

extern GUI_CONST_STORAGE GUI_BITMAP bmc;

extern GUI_CONST_STORAGE GUI_BITMAP bmo;
...
static int s_i32DoorTextX;
static int s_i32DoorTextY;
static int s_i32DoorTextCX;
static int s_i32DoorTextCY;
...
void NVT_SetupRes(void)
{
    ...
    if (hMemFloorNumber == 0)
        hMemFloorNumber = GUI_MEMDEV_CreateFixed32(0, 0, bmf0.XSize, (bmf0.YSize + 7 + bmf0.YSize));
    ...
    //
    s_i32ArrowX = (0);
    s_i32ArrowY = ((YSIZE_PHYS_OSD - bmu00.YSize) / 2);
    //
    // Floor number position for units digit in bounding box
    //
    s_i32FloorNumberX = (bmu00.XSize + 43);
    s_i32FloorNumberY = ((YSIZE_PHYS_OSD - bmf0.YSize) / 2);
    //
    // Floor number position for tens digit in bounding box
    //
    s_i32FloorNumber10X = (bmu00.XSize + 43);
    s_i32FloorNumber10Y = (bmf0.YSize + 7);
    s_i32FloorNumber01X = (bmu00.XSize + 43);
    s_i32FloorNumber01Y = 0;
    //
    // Door status position in bounding box
    //
    s_i32DoorTextX = (bmu00.XSize + 43 + bmf0.XSize + 47);
    s_i32DoorTextY = ((YSIZE_PHYS_OSD - bmo.YSize) / 2);
    s_i32DoorTextCX = (bmu00.XSize + 43 + bmf0.XSize + 47);
    s_i32DoorTextCY = ((YSIZE_PHYS_OSD - bmc.YSize) / 2);
}
//
...
GUI_CONST_STORAGE GUI_BITMAP *pbmu[35];

GUI_CONST_STORAGE GUI_BITMAP *pbmd[35];
...
void NVT_ShowFloorNumber(const GUI_BITMAP * pBM, int dx, int dy, int Mag)
{
    ...
    GUI_SetBkColor(DEF_OSD_COLORKEY);
    GUI_ClearRect(dx, dy, dx + pBM->XSize - 1, dy + pBM->YSize + 7 + pBM->YSize - 1);
...
void MainTask(void)
{
    ...
    while (1)
    {
        ...
        GUI_DrawBitmap(&bmo, s_i32DoorTextX, s_i32DoorTextY);
        ...
        GUI_Delay(500);
#else
        _WAV_Decode("C:\\WAV\\FL\\001.wav");
        _WAV_Decode("C:\\WAV\\DO\\open001.wav");
#endif
        //
        ...
        GUI_SetBkColor(DEF_OSD_COLORKEY);
        GUI_ClearRect(s_i32DoorTextX, s_i32DoorTextY, s_i32DoorTextX + bmo.XSize - 1, s_i32DoorTextY + bmo.YSize - 1);

        GUI_DrawBitmap(&bmc, s_i32DoorTextCX, s_i32DoorTextCY);
        //
        // Ping-pong buffer flow: switch canvas
        //
        NVT_Rot();
        ...
        GUI_Delay(500);
#else
        _WAV_Decode("C:\\WAV\\DO\\close001.wav");
#endif
        if (g_flagM0S0 == 0)
        {
            GUI_MEMDEV_Select(hMemM0S0);
        ...
        Mag = 500;  /* Scaling factor 500 mean 0.5X */
        fiFlag = 0;
        ...
        while (1)
        {
            ...
            //
            // Key action to load & run lang_en FW
            //
            if (key != 0)
            {
                if (key == 1)
            ...
            if (ui == 10)
            {
                ...
                GUI_Clear();
                if (fi < 10 )
                    GUI_DrawBitmap(pbmf[fi], 0, (bmf0.YSize + 7 + bmf0.YSize - bmf0.YSize) / 2);
                else
                {
                    GUI_DrawBitmap(pbmf[fi / 10], 0, (bmf0.YSize + 7));
            ...
            if (fiFlag == 1)
            {
                NVT_ShowFloorNumber(pbmf[0], s_i32FloorNumberX, 0, Mag);
                NVT_Rot();
                Mag += 50;
                if (Mag > 1000)
                {
                    NVT_ShowFloorNumber(pbmf[0], s_i32FloorNumberX, 0, 1000);   /* scaling factor 1000 means 1.0X */
                    NVT_Rot();
                    Mag = 500;
                    fiFlag = 0;
                    if (fi == 10)
        ...
        while (1)
        {
            ...
            if (fiFlag == 1)
            {
                NVT_ShowFloorNumber(pbmf[fi], s_i32FloorNumberX, 0, Mag);
                NVT_Rot();
                Mag += 50;
                if (Mag > 1000)
                {
                    NVT_ShowFloorNumber(pbmf[fi], s_i32FloorNumberX, 0, 1000);
                    NVT_Rot();
                    Mag = 500;
                    fiFlag = 0;
                    if (fi == 1)
...
__NVT_EXIT__:
    ...
    sprintf(s_szFileNameDst, "C:\\2onprog.bin");
    ...
    sprintf(s_szFileNameSrc, "C:\\3onprog.bin");
    fsAsciiToUnicode(s_szFileNameSrc, s_szFileNameSrc2, TRUE);
