...
void NVT_SetupRes(void)
{
    ...
    if (hMemFloorNumber == 0)
        hMemFloorNumber = GUI_MEMDEV_CreateFixed32(0, 0, (bmf0.XSize + 5 + bmf0.XSize), bmf0.YSize);
    ...
    //
    s_i32FloorNumberX = ((bmf0.XSize + 5 + bmf0.XSize - bmf0.XSize) / 2);
    s_i32FloorNumberY = (0);
    //
    // Floor number position for tens digit in bounding box
    //
    s_i32FloorNumber10X = (0);
    s_i32FloorNumber10Y = (0);
    s_i32FloorNumber01X = (bmf0.XSize + 5);
    s_i32FloorNumber01Y = (0);
    //
    // Arrow position in bounding box
    //
    s_i32ArrowX = (bmf0.XSize + 5 + bmf0.XSize + 45);
    s_i32ArrowY = (0);
}
//
...
GUI_CONST_STORAGE GUI_BITMAP *pbmu[20];

GUI_CONST_STORAGE GUI_BITMAP *pbmd[20];
...
void NVT_ShowFloorNumber(const GUI_BITMAP * pBM, int dx, int dy, int Mag)
{
    ...
    GUI_SetBkColor(DEF_OSD_COLORKEY);
    GUI_ClearRect(dx, dy, dx + pBM->XSize + 5 + pBM->XSize - 1, dy + pBM->YSize - 1);
...
void MainTask(void)
{
    ...
    while (1)
    {
        ...
        GUI_Delay(500);
#else
        _WAV_Decode("D:\\WAV\\1\\FL\\elevator.wav");
        _WAV_Decode("D:\\WAV\\1\\DO\\open001.wav");
#endif
        //
        ...
        GUI_Delay(500);
#else
        _WAV_Decode("D:\\WAV\\1\\DO\\close001.wav");
#endif
        ui = 0;
        fi = 1;
        ...
        while (1)
        {
            ...
            //
            // Key action to load & run lang_zh FW
            //
            if (key != 0)
            {
                if (key == 1)
            ...
            if (fiFlag == 0)
            {
                ...
                GUI_Clear();
                if (fi < 10 )
                    GUI_DrawBitmap(pbmf[fi], ((bmf0.XSize + 5 + bmf0.XSize - bmf0.XSize) / 2), 0);
                else
                {
                    GUI_DrawBitmap(pbmf[fi / 10], 0, 0);
                    GUI_DrawBitmap(pbmf[fi % 10], (bmf0.XSize + 5), 0);
                }
                fiFlag = 1;
            }
            ...
            if (fiFlag == 1)
            {
                if (Mag > 1000)
                {
                    NVT_ShowFloorNumber(pbmf[0], 0, 0, 1000);   /* scaling factor 1000 means 1.0X */
                    fiFlag = 2;
                }
                else
                    NVT_ShowFloorNumber(pbmf[0], 0, 0, Mag);
                Mag += 100;
            }
            else if (fiFlag >= 2)
            {
        ...
        while (1)
        {
            ...
            if (fiFlag == 1)
            {
                if (Mag > 1000)
                {
                    NVT_ShowFloorNumber(pbmf[0], 0, 0, 1000);
                    fiFlag = 2;
                }
                else
                    NVT_ShowFloorNumber(pbmf[0], 0, 0, Mag);
...
__NVT_EXIT__:
    ...
    sprintf(s_szFileNameDst, "C:\\1onprog.bin");
    ...
    sprintf(s_szFileNameSrc, "C:\\2onprog.bin");
    fsAsciiToUnicode(s_szFileNameSrc, s_szFileNameSrc2, TRUE);
