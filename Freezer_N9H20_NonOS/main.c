...
void MainTask(void)
{
    WM_HWIN hWinMain1, hWinSetting1, hWinFreezer1;
    ...
    while (1)
    {
        ...
        {
            g_Main1Flag = 0;
            WM_DeleteWindow(hWinSetting1);
            WM_ShowWindow(hWinMain1);
        }
        if (g_Main1Flag == 2)
        {
            g_Main1Flag = 0;
            WM_DeleteWindow(hWinFreezer1);
            WM_ShowWindow(hWinMain1);
        }
        if (g_Setting1Flag == 1)
        {
            g_Setting1Flag = 0;
            WM_HideWindow(hWinMain1);
            hWinSetting1 = CreateSetting1();
        }
        if (g_Freezer1Flag == 1)
        {
            g_Freezer1Flag = 0;
            WM_HideWindow(hWinMain1);
            hWinFreezer1 = CreateFreezer1();
        }
    }
}
...
int main(void)
{
    ...
    if (hFile < 0)
    {
        ...
        GUI_Init();
        ts_calibrate(LCD_YSIZE, LCD_XSIZE);
        ts_writefile(hFile);
    }
    else
    {
